...
class Settings(BaseSettings):
    source_host: str = "https://localhost:5665"
    source_user: str = ""
    source_passwd: str = ""
    source_hostgroups = "XYZ"

    sink_host: str = "https://localhost:5665"
    sink_user: str = ""
    sink_passwd: str = ""
    sink_vars_prefix = "pr_"
...
def process_replication() -> None:
    for hostgroup in settings.source_hostgroups.split(","):
...
def collect_from_source(hostgroup: str):
    ...
    source = Source()
    source.host = settings.source_host
    source.user = settings.source_user
    source.passwd = settings.source_passwd
...
def push_to_sink(hosts: Hosts, services: Services):
    sink = Sink()
    sink.host = settings.sink_host
    sink.user = settings.sink_user
    sink.passwd = settings.sink_passwd
    sink.vars_prefix = settings.sink_vars_prefix
